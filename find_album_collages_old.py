...
class OrpheusAlbumSearcher:
    ...
    async def display_albums(
        ...
        for i in range(start_idx, end_idx):
            ...
            # Collages are already shown above if requested
    ...
    async def interactive_search(self, albums: List[Dict], show_collages: bool = False, page_size: int = 1):
        ...
        while current_idx < len(filtered_albums):
            ...
            next_idx = await self.display_albums(filtered_albums, current_idx, page_size, show_collages, show_tracks)
            
            # Show pagination info
            remaining = len(filtered_albums) - next_idx
            print(f"👋  Showing {next_idx} of {len(filtered_albums)} releases ({remaining} remaining)")
            print("-" * 60)
            
            print("\n🎯  Options:")
            options_line1 = []
            if remaining > 0:
                options_line1.append("n = Next release")
            if show_tracks:
                options_line1.append("t = Hide tracks")
            else:
                options_line1.append("t = Show tracks")
            options_line1.append("q = Quit")
            print("   " + " | ".join(options_line1))
            
            print("   d <#> <fmt> = Download (e.g. 'd 1 a2')")
                ...
                if choice == 'q':
                    ...
                    if len(parts) == 3:
                        try:
                            ...
                            print("❌ Invalid format! Use: d [album_num] [release_format] (e.g., 'd 1 a2')")
                    else:
                        print("❌ Invalid format! Use: d [album_num] [release_format] (e.g., 'd 1 a2')")
                else:
                    print("❌ Invalid choice. Use 'n' for next, 't' for tracks, 'd [album] [format]' to download, or a number to filter.")
                    continue
...
async def main():
    ...
    async with OrpheusAlbumSearcher() as searcher:
        ...
        if args.interactive:
            await searcher.interactive_search(sorted_albums, args.show_collages, page_size=1)
        else:
