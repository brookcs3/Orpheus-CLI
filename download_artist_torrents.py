#!/usr/bin/env python3
"""
Download all torrent files from an Orpheus artist's discography
Based on download_collage_torrents.py but adapted for artist endpoint
"""
...
class OrpheusArtistDownloader:
    ...
    async def get_artist_info(self, artist_id: int) -> Dict:
        """Get artist information and all their releases"""
        url = f"{self.base_url}/ajax.php"
        headers = {
            'Authorization': f'token {self.api_key}',
            'User-Agent': 'ArtistDownloader/1.0'
        }

        params = {
            'action': 'artist',
            'id': artist_id,
            'artistreleases': 1  # Only include releases where this is the main artist
        }
    ...
    def convert_artist_data_to_torrents(self, artist_data: Dict) -> List[Dict]:
        """Convert artist API response to torrent list format (similar to collage format)"""
        all_torrents = []
        artist_name = artist_data.get('name', 'Unknown Artist')
        
        # Process each torrent group (album/release)
        for group in artist_data.get('torrentgroup', []):
            group_id = group.get('groupId')
            album_name = group.get('groupName', '')
            year = group.get('groupYear', '')
            
            # Process each torrent in the group
            for torrent in group.get('torrent', []):
                torrent_info = {
                    ...
                    'remastered': torrent.get('remastered', False),
                    'remasterYear': torrent.get('remasterYear', 0),
    ...
    async def download_artist_torrents(self, artist_id: int, output_dir: str = None,
        ...
        """Download all torrents from an artist's discography"""
        ...
        if not artist_info:
            print("❌ Could not fetch artist info")
            return {"success": False, "error": "Could not fetch artist info"}

        artist_name = artist_info.get('name', f'Artist_{artist_id}')
        print(f"🎯 Artist: {artist_name}")
        print(f"   Total Groups: {artist_info.get('statistics', {}).get('numGroups', 0)}")
        print(f"   Total Torrents: {artist_info.get('statistics', {}).get('numTorrents', 0)}")
        print(f"🎵 Encoding preferences: {' → '.join(encoding_prefs)}")

        # Set up output directory
        if not output_dir:
            output_dir = f"artist_{artist_id}_{self.sanitize_filename(artist_name)}"

        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        # Convert artist data to torrent list
        all_torrents = self.convert_artist_data_to_torrents(artist_info)

        if not all_torrents:
            print("❌ No torrents found for artist")
            return {"success": False, "error": "No torrents found"}

        print(f"📋 Found {len(all_torrents)} total torrents")
        ...
        with open(info_file, 'w') as f:
            json.dump({
                ...
                'torrentgroups': all_torrents  # Use same key name as collage version
            }, f, indent=2)
...
async def main():
    parser = argparse.ArgumentParser(description="Download all torrent files from an Orpheus artist's discography")
    parser.add_argument('artist_id', type=int, help='Artist ID to download')
    ...
    async with OrpheusArtistDownloader(args.api_key) as downloader:
        result = await downloader.download_artist_torrents(
